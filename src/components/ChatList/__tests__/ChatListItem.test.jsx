...
describe('ChatListItem', () => {
  ...
  it('is correctly rendered to the screen', () => {
    render(<ChatListItem name={activeUserData.name}
  ...
  it('Click chat list item changes it to selected', () => {
    render(<ChatListItem testId="under_test" name={activeUserData.name}
                         active={activeUserData.active}
                         lastMessageText={activeUserData.lastMessage}
                         lastMessageTime={activeUserData.lastMessageTime}
                         onClick={() => null}/>)
    ...
    expect(screen.getByTestId('ButtonBase')).toHaveStyle({background: "#ECF3FF"});
  });

  it('produces the correct initial snapshot', () => {
    const tree = renderer.create(<ChatListItem testId="under_test" name={activeUserData.name}
                                               active={activeUserData.active}
                                               lastMessageText={activeUserData.lastMessage}
                                               lastMessageTime={activeUserData.lastMessageTime}
                                               onClick={() => null}/>).toJSON();
